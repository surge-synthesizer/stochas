...
void SeqFileDialog::fileDoubleClicked(const File &) { closeDialog(true); }
...
void SeqFileDialog::doSetup(int mode)
{
    ...
    case SEQ_FILE_SAVE_NOTES:
        ...
        mFileFilter = std::unique_ptr<WildcardFileFilter>(
            new WildcardFileFilter("*.stnote", "", "note files"));
        saveMode = true;
        break;
    ...
    case SEQ_FILE_LOAD_MIDI:
        mFileFilter =
            std::unique_ptr<WildcardFileFilter>(new WildcardFileFilter("*.*", "", "midi files"));
    ...
    case SEQ_FILE_LOAD_PATCH:
        mFileFilter = std::unique_ptr<WildcardFileFilter>(
            new WildcardFileFilter("*.stochas", "", "note files"));
    ...
    mBrowser = std::unique_ptr<FileBrowserComponent>(
        new FileBrowserComponent(fflags, mLastLocation, mFileFilter.get(), 0));
    ...
    : SeqModalDialog(glob, SEQCTL_FILEDIALOG, parent, 600, 500)
{
    ...
    mLblMain =
        std::unique_ptr<Label>(addStdLabel("Select a file")); // will be changed as appropriate
...
SeqFileDialog::~SeqFileDialog()
{
    ...
    // TODO is this still the case with new JUCE? also I switched this from ScopedPointer to
    // unique_ptr
    if (mBrowser)
    {
        delete mBrowser.release();
