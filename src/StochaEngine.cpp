...
bool StochaEngine::addMidiEvent(int startSamples, int8_t note, int8_t velo, int8_t chan,
                                int numSamples)
{
    int transp, i;
    ...
    for (i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
    {
        ...
        if (velo && cur->mNote == note && cur->mChan == chan && cur->mVelo == 0 &&
...
StochaEngine::StochaEngine()
    : mNumActiveNoteOnEvents(0), mNumActiveNoteOffEvents(0), mCurrentStepPosition(INVALIDPOS),
      mRealStepPosition(INVALIDREALPOS), mSeq(nullptr), mLayer(-1), mOldSeed(0), mOldSeq(0),
      mOldStepPosInTrack(0.0), mPlayStartPosition(0.0)
...
StochaEngine::~StochaEngine() { resetMappingSchema(); }
...
bool StochaEngine::playPositionChange(
    int samples_per_step, // samples per sequencer step
    ...
    int samples_until)    // num samples between now and next step pos
{
    ...
    if (!muted)
    {
        ...
        for (int i = 0; i < playcount; i++)
        {
            ...
            if (whichNote != -1)
            { // -1 is the "off" note, which we ignore
                int gruuv;
                int8_t velocity = sd->getVel(mMulti[i].rowToPlay, position, pat);
                int humanpos, humanvel, humanlen;

                // get additional length to play, or get number of times to trigger (0 means play
                // one step, so 1 means play one additional step. anything negative is for multiple
                // triggers which the step will be divided into that many)
                ...
                { // single trigger with optional additional length to play
                    // length here is in samples and represents whole steps.
                    lensize =
                        samples_per_step * steplen; // will be 0 in the case of no additional length
                ...
                if (velocity &&
                    ((humanvel = mOverrideVeloVariance.get(sd->getHumanVelocity())) != 0))
                {
                ...
                if (lensize)
                {
                    for (int j = 0; j < numtriggers; j++)
                    {
                        if (!addMidiEvent(gruuvOffset,
                                          ...
                                          lensize)) // size in steps + size of one step (which is
                                                    // adjusted by duty cycle)
                            return false;
...
int StochaEngine::getRandomSingle(int position)
{
        ...
        if (isMandatory(position, i, &neg, pat))
        {
            if (!neg)
            {
                ...
                if (sd->getProb(i, position, pat) > 0)
                    allMandAreZero =
                        false; // we have at least one mandatory note that is not prob-0 ("never")
            }
            else
                mandatoryOff.set(i);
        }
    ...
    // Cycle 1 adds up probabilities for selected notes and then generates a random number based on
    // that. Cycle 2 does the same thing, but exits as soon as a note's prob exceeds the random
    // number and returns that note
            ...
            if (!mandatoryOff.test(i))
            {
                ...
                if (prob != SEQ_PROB_OFF)
                {
                    ...
                    else if (useMand && allMandAreZero)
                    {
                        ...
                        // so in this case, we had all mandatory notes are "never". so they all
                        // should get equal chance to play (as opposed to one or more of them having
                        // a probability in which case the normal probability selection is followed)
                        maxprob += SEQ_PROB_ON;
                    }
                } // if prob != SEQ_PROB_OFF
...
int StochaEngine::getRandomMulti(int position, int maxpoly)
{
    ...
    for (i = startrow; i < SEQ_MAX_ROWS; i++)
    {
        ...
        // determine if it's mandatory (it must also be on. can't play a cell that's not on even if
        // mand.) so it may have a prob of 0 which would allow it to play in this case but not
        // otherwise.
...
void StochaEngine::setAutomationParameterValue(int paramId, int value)
{
    if (value == SEQ_AUT_DEFAULT_VALUE_DESIG)
    {
        ...
        switch (paramId)
        {
        case SEQ_AUT_CLOCKDIV:
            targ = &mOverrideSpeed;
            break;
        case SEQ_AUT_NUMSTEPS:
            targ = &mOverrideNumSteps;
            break;
        case SEQ_AUT_STEPS_PER_MEASURE:
            targ = &mOverrideStepsPerMeasure;
            break;
        case SEQ_AUT_NOTE_LENGTH:
            targ = &mOverrideDutyCycle;
            break;
        case SEQ_AUT_POS_VARIANCE:
            targ = &mOverridePosVariance;
            break;
        case SEQ_AUT_VELO_VARIANCE:
            targ = &mOverrideVeloVariance;
            break;
        case SEQ_AUT_LENGTH_VARIANCE:
            targ = &mOverrideLengthVariance;
            break;
        case SEQ_AUT_MUTED:
            targ = &mOverrideMute;
            break;
        case SEQ_AUT_OUTPUT_CHANNEL:
            targ = &mOverrideOutputChannel;
            break;
        case SEQ_AUT_MAX_POLY:
            targ = &mOverrideMaxPoly;
            break;
        case SEQ_AUT_POLY_BIAS:
            targ = &mOverridePolyBias;
            break;
        case SEQ_AUT_CURRENT_PATTERN:
            targ = &mOverridePattern;
            break;
        case SEQ_AUT_TRANSPOSE:
            targ = &mOverrideTranspose;
            break;
    ...
    case SEQ_AUT_MUTED: // tested
        performMidiMapAction(SEQMIDI_ACTION_MUTE,
                             value ? SEQMIDI_VALUE_MUTE_MUTE : SEQMIDI_VALUE_MUTE_UNMUTE);
        break;
...
void StochaEngine::performMidiMapAction(int action, int value)
{
    ...
    case SEQMIDI_ACTION_CHGPAT:
        ...
        { // 1 based pattern number incoming
    ...
    case SEQMIDI_ACTION_SPEED:
    {
        ...
        if (value == SEQMIDI_VALUE_SPD_DBL)
        { // mult current by 2 if possible
            if (cs < SEQ_MAX_CLOCK_DIV)
                mOverrideSpeed.override(cs * 2);
        }
        else if (value == SEQMIDI_VALUE_SPD_HALF)
        { // div current by 2 if possible
...
void StochaEngine::rebuildMappingSchema()
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        if (mm->mAction != SEQMIDI_ACTION_INVALID && mm->mTarget != 0 && mm->mValue != 0 &&
...
void StochaEngine::quiesceMidi(bool moveNoteOffs)
{
    int i;
    ...
    for (i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
    {
        ...
        { // is in use?
            if (cur->mVelo > 0)
            { // is note on? delete all note on events that exist
    ...
    if (moveNoteOffs)
    { // find any remaining note off events and move them forward
        for (i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
        {
            cur = &mEvents[i];
            if (cur->mNumSamples != -1)
            { // is in use?
                jassert(cur->mVelo ==
                        0); // it better be a note off, because we removed the note on's above
...
bool StochaEngine::processBlock(
    double beatPosition,   // which quarter measure we are on
...
)
{
    ...
    // beatPosition - (could be fractional, indicates where in the track we are in quarters of a
    // measure )
    //   each whole number is a beat (1/4 of a measure)
    //   note that the doc for this ppqPosition seems wrong, it seems to indicate "pulses per
    //   quarter note"

    // mPlayStartPosition - normally 0, but might represent a position where PLAY was manually
    // started. We might be positioned before this (eg if the DAW looped back around after play was
    // nmanually started) in which case we need to offset it so we don't get a negative value and
    // also so that we continue to line up when the daw reaches the manual play start position again
    double adjBeatPos;
    ...
    if ((steppos_in_track < mOldStepPosInTrack) ||
        (mCurrentStepPosition != INVALIDPOS &&
         steppos_in_track >= (double)(mCurrentStepPosition + 1)))
    {
    ...
    // 1. determine the next step position that we need to deal with (mCurrentStepPosition is the
    // last one we played)
    if (mCurrentStepPosition == INVALIDPOS)
    {
        ...
        {
            // in cubase (and maybe others), the step position can be negative when playing from 0
            // position
        ...
        else
        {
            ...
            // If we are less than half-way between one step and another step, we will play the
            // first step (eg for step 0, if we are at 0.4 we will still play step 0 even though
            // it's actually in the past). this is to solve the issue of looping when a sample block
            // does not start at the beginning of the loop. if this becomes problematic for people
            // (can't see how it would) then we could always round down at eg 0.1 and round up
            // otherwise. i'm just trying to handle the case of eg combination of a very large block
    ...
    // 2. determine the step position (frac) of either the halfway point before that position, or if
    // we
    //    are past the halfway point to that position, then our position is the current step (frac)
    //    position
    ...
    while (next_half_step_pos < last_step_pos_in_block)
    {
        ...
        // 4. if so, determine how many samples are occurring between now and the next _step_
        // position
...
void StochaEngine::setPlaybackStartPosition(double pos) { mPlayStartPosition = pos; }

bool StochaEngine::getMidiEvent(int numSamplesInBlock, int *pos, int8_t *note, int8_t *velo,
                                int8_t *chan)
{
    StochaEvent *cur = nullptr;
    bool rc = true;
    ...
    // we need to put these out before any note on events in case we get a note-on and note-off at
    // same sample position. the note off needs to come out first
    if (mNumActiveNoteOffEvents)
    {
        for (int i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
        {
            cur = &mEvents[i];
            if (cur->mVelo == 0 && cur->mNumSamples != -1)
            { // it's occupied
                if (cur->mNumSamples < numSamplesInBlock)
                { // it needs to be sent out with this block
                    *pos = cur->mNumSamples;
                    *note = cur->mNote;
                    *velo = cur->mVelo;
                    *chan = cur->mChan;
    ...
    if (mNumActiveNoteOnEvents)
    {
        for (int i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
        {
            ...
            {   // it's occupied
                ...
                // when it loops back around and we get an extra note. I'm guessing that its because
                // we are putting out a note in a block prior to where it actually should be put
                // out. This may be due to rounding(?) note that this issue only crops up with
                // certain block sizes and tempos obviously because it happens (i think) when the
                // event is at the very end of the block and should actually be fired in the
                // following block.
                if (cur->mNumSamples < numSamplesInBlock - 1)
                { // it needs to be sent out with this block
                    *pos = cur->mNumSamples;
                    *note = cur->mNote;
                    *velo = cur->mVelo;
...
void StochaEngine::doneBlock(int numSamplesInBlock)
{
    int x;
    for (int i = 0; i < SEQ_MAX_MIDI_EVENTS; i++)
    {
...
bool StochaEngine::incomingMidiData(int type,      // note on, off or cc
