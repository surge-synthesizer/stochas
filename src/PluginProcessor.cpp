...
#endif
      mPlaying(false), mEditorState(0), mPPQAdjust(0.0f), mNotifier(this), mRecordingMode(false),
      mMPBrequestArmed(false), mMPBstate(MPBstopped), mMPBStartPosition(0.0)
{
    ...
    {
        // 0-based!
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_CLOCKDIV, 0,
                                                   SEQ_NUM_CLOCK_DIVS - 1, i, "speed"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_NUMSTEPS, SEQ_MIN_STEPS,
                                                   SEQ_MAX_STEPS, i, "steps"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_STEPS_PER_MEASURE, SEQ_MIN_STEPS,
                                                   SEQ_DEFAULT_NUM_STEPS, i, "steps/measure"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_NOTE_LENGTH, SEQ_DUTY_MIN,
                                                   SEQ_DUTY_MAX, i, "note length"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_POS_VARIANCE, 0,
                                                   SEQ_MAX_HUMAN_POSITION, i, "pos variance"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_VELO_VARIANCE, 0,
                                                   SEQ_MAX_HUMAN_VELOCITY, i, "velo variance"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_LENGTH_VARIANCE, 0,
                                                   SEQ_MAX_HUMAN_LENGTH, i, "len variance"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_MUTED, 0, 1, i, "muted"));
        addAutParam(
            new SeqAudioProcessorParameter(this, SEQ_AUT_OUTPUT_CHANNEL, 1, 16, i, "midi channel"));
        addAutParam(
            new SeqAudioProcessorParameter(this, SEQ_AUT_MAX_POLY, 1, SEQ_MAX_ROWS, i, "max poly"));
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_POLY_BIAS, SEQ_POLY_BIAS_MIN,
                                                   SEQ_POLY_BIAS_MAX, i, "poly bias"));
        addAutParam(
            new SeqAudioProcessorParameter(this, SEQ_AUT_TRANSPOSE, SEQMIDI_VALUE_TRANSPOSE_MIN,
                                           SEQMIDI_VALUE_TRANSPOSE_MAX_AUT, i, "transpose"));
        // 0-based!
        addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_CURRENT_PATTERN, 0,
                                                   SEQ_MAX_PATTERNS - 1, i, "pattern"));
    }
    addAutParam(new SeqAudioProcessorParameter(this, SEQ_AUT_GLOBAL_SWING, SEQ_MIN_SWING,
                                               SEQ_MAX_SWING, -1, "swing"));
    ...
    addParameter(mDummyParam = new AudioParameterFloat("_rsvd", "reserved",
...
const String SeqAudioProcessor::getName() const { return JucePlugin_Name; }
...
double SeqAudioProcessor::getTailLengthSeconds() const { return 0.0; }
...
int SeqAudioProcessor::getCurrentProgram() { return 0; }

void SeqAudioProcessor::setCurrentProgram(int) {}

const String SeqAudioProcessor::getProgramName(int) { return String(); }
...
void SeqAudioProcessor::rebuildMiniMidiMap()
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if ((mm->mAction == SEQMIDI_ACTION_PLAYBACK || mm->mAction == SEQMIDI_ACTION_RECORD) &&
            mm->mValue != 0 && mm->mChannel != 0 && mm->mNote >= 0)
        {
...
void SeqAudioProcessor::handleMiniMidiMap(int type, int8_t number, int8_t chan, int8_t)
{
    MiniMidiMapItem *mi;
...
void SeqAudioProcessor::handleIncomingMidi(bool currentlyPlaying, bool startingPlayback,
                                           MidiBuffer &midiMessages, MidiBuffer &processedMidi,
                                           MidiBuffer &recordedNotes)
{
    ...
    else
    {
        ...
        for (const MidiMessageMetadata it : midiMessages)
        {
            ...
            if (currentlyPlaying)
            {   // currently playing recording or looping
                ...
                // if recording (which doesn't care whether we are responding or not, if the user
                // hits record, it's assumed they want to respond to midi) then build up our buffer
                // with those notes
...
void SeqAudioProcessor::checkIncomingMidiForStartStop(MidiBuffer &midiMessages)
{
...
void SeqAudioProcessor::dispatchRecordedMidiNotes(MidiBuffer &midiNoteData)
{
    ...
    for (const MidiMessageMetadata it : midiNoteData)
    {
        ...
        {                // stop rec and update sequence data
            ...
            if (mMidiRecord[noteNum].vel)
            {
                ...
                // calculate total length (stepPos will always be > what we stored as it's position
                // in daw) (should always result in a length of at least 1)
                length = stepPos - mMidiRecord[noteNum].stepPos + 1;

                // determine whether the two fractions on either end would cause the length to
                // quantize upward or downward
                ...
                // finally, if any of the above cause length to go below 0, we will set it to 0
                // ensuring we actually have a step that gets placed
                if (length < 0)
                    length = 0;
...
void SeqAudioProcessor::checkforUIIncomingData(MidiBuffer &processedMidi)
{
    ...
    while (mIncomingData.readFromFifo(&fifoData1, &fifoData2, &fifoData3))
    {
        ...
        case SEQ_MIDI_NOTEOFF:
        {
            ...
            MidiMessage m;
            // note that this currently ignores automation override of midi channel (which I think
            // is fine)
            if (fifoData1 == SEQ_MIDI_NOTEOFF)
            { // note off
                m = MidiMessage::noteOff(chan, (int)note, (uint8)0);
                processedMidi.addEvent(m, 0);
            }
            else if (fifoData1 == SEQ_MIDI_NOTEON)
            { // note on
        ...
        case SEQ_REFRESH_MAP_MSG:
            // ui has changed the midi mapping, so we need to tell our stocha's to rebuild their
            // schema
            for (int i = 0; i < SEQ_MAX_LAYERS; i++)
            {
                mStocha[i].resetMappingSchema();
...
inline bool SeqAudioProcessor::determinePlaybackState(int apm, bool playingInDaw,
                                                      double beatsperbar, double samplerate,
                                                      ...
                                                      int samplesperblock)
{
    ...
    case MPBrequested: // user requested start
        if (playingInDaw)
        { // only honor this request if we are playing in the daw
            ...
            if (apm != SEQ_PLAYMODE_INSTANT)
            {
                ...
                switch (apm)
                {
                case SEQ_PLAYMODE_STEP: // right now this is in daw 16th notes always. (stocha
                                        // engines may have different step sizes)
                ...
                else if (divpos < ((double)roundedDiv) + fudge)
                {
                    ...
                    // this should handle the case where our position is very soon after start of
                    // bar
                    mMPBstate = MPBstarted;
                    // round this to the division boundary. if there are any stocha notes at this
                    // position they will be skipped, as they occurred in the past
                    mMPBStartPosition = (double)roundedDiv / multiplier;
                }
            }
            else
            { // apm = SEQ_PLAYMODE_INSTANT
                mMPBstate = MPBstarted;
    ...
    case MPBstarted: // already started
        if (!playingInDaw)
        { // but we are not playing anymore in the daw
            mNotifier.setPlaybackState(SeqProcessorNotifier::off);
            mMPBstate = MPBstopped;
            mMPBStartPosition = 0.0;
        }
...
void SeqAudioProcessor::processBlock(AudioSampleBuffer &buffer, MidiBuffer &midiMessages)
{
    ...
    else
    {
        ...
        areWePlaying =
            determinePlaybackState(seq->getAutoPlayMode(), posinfo.isPlaying, beatsperbar,
                                   samplerate, posinfo.bpm, posinfo.ppqPosition, samplesperblock);
    }

    // determine whether we are transitioning from playing to not playing
    // or vice versa
    if (!areWePlaying)
    { // if we are stopped playback
...
#endif
        if (mPlaying)
        {                     // but was playing before this, then
    ...
    { // if we are actually playing
        ...
        if (!mPlaying)
        { // and was not playing before this
            mPlaying = true;
...
#error "Need to add some prime numbers to the array now"
    ...
    if (mPlaying)
    {
        ...
        for (i = 0; i < SEQ_MAX_LAYERS; i++)
        {
            ...
            mStocha[i].processBlock(posinfo.ppqPosition, // which quarter measure we are on
                                    samplerate, samplesperblock, posinfo.bpm, beatsperbar
#ifdef CUBASE_HACKS
                                    ,
                                    mCubaseAtRestPos
#endif
    ...
    if (mPlaying || stoppingPlayback)
    {
        ...
        for (i = 0; i < SEQ_MAX_LAYERS; i++)
        {
            while (mStocha[i].getMidiEvent(samplesperblock, &midi_pos, &midi_note, &midi_velo,
                                           &midi_chan))
            {
                MidiMessage m;
                if (midi_velo)
...
*/
void SeqAudioProcessor::getStateInformation(MemoryBlock &block)
{
    SeqPersist persist;
...
void SeqAudioProcessorParameter::setValue(float newValue)
{
    ...
    mNotify->automationParameterHasChanged(
        mParamId, mValue == mRangeLo - 1 ? SEQ_AUT_DEFAULT_VALUE_DESIG : mValue, mLayerNumber);
}
...
SeqAudioProcessorParameter::SeqAudioProcessorParameter(AutParamNotify *notify, int paramId,
                                                       int rangeLo, int rangeHi, int layerNum,
                                                       const String &name)
