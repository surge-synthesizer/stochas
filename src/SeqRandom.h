...
class SeqRandom
{
  ...
  public:
    ...
    struct Statistics
    {
        ...
        int generations;    // how many times we generated a random number for an intermediate value
                            // (ie we didn't use it)
        int seq_saves;   // how many times we avoided having to generate intermediate random values
                         // because positions were in sequence
        int frame_saves; // how many times we avoided having to generate intermediate values because
                         // we already had a frame
