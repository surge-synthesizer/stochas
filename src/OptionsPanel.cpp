...
OptionsPanel::OptionsPanel(SeqGlob *glob, CptNotify *parent)
    : Component("optionsPanel"), mGlob(glob), mMonoPoly(glob, SEQCTL_OPT_MP_TOGGLE, this, "tgMP"),
      ...
      mPolyBias(glob, SEQCTL_OPT_POLYBIAS, this, "numPolyBias"), mScaleList("cbbScale"),
      mKeyList("cbbKey"), mOctaveList("cbbOct"),
      mBtnLoadCustom(glob, SEQCTL_OPT_LOAD_CUSTOM, this, "btnLdCust"),
      mBtnSaveCustom(glob, SEQCTL_OPT_SAVE_CUSTOM, this, "btnSvCust"),
      mBtnStdCustom(glob, SEQCTL_OPT_STD_TO_CUSTOM, this, "btnStdCust"), mParent(parent)
{
    int i = 0, len = 0;
    juce::Colour txtclr = mGlob->mEditorState->getColorFor(EditorState::background).contrasting(.5);
...
void OptionsPanel::cptValueChange(int cptId, int id)
{
    ...
    case SEQCTL_OPT_PATLEN_NUMCTL: // steps per pattern
        if (sl->getNumSteps() != id)
        { // number of steps has changed
            sl->setNumSteps(id);
            if (id <= SEQ_DEFAULT_NUM_STEPS)
                e.setVisibleStep(0); // switch to page 1 view (as we might have been on "view all"
                                     // mode which would cause it to stretch)
        }
        break;
...
void OptionsPanel::comboSetup(ComboBox *cb)
{
...
void OptionsPanel::findItemInCombo(ComboBox *cb, String txt)
{
    int i;
