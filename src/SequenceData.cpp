...
int SeqProcessorNotifier::getPlayPosition(int layer) { return mPlayPosition[layer].get(); }

int SeqProcessorNotifier::getCurrentPattern(int layer) { return mCurrentPattern[layer].get(); }

bool SeqProcessorNotifier::getMidiEventOccurred(int8_t *type, int8_t *channel, int8_t *number,
                                                int8_t *value)
{
...
bool SeqProcessorNotifier::getMuteState(int layer) { return mMuteState[layer].get() == 1; }

int64 SeqProcessorNotifier::getRandomSeed() { return mRandomSeed.get(); }
...
void SeqProcessorNotifier::setMidiEventOccurred(int8_t type, int8_t channel, int8_t number,
                                                int8_t value)
{
...
void SeqProcessorNotifier::setPlayPosition(int layer, int val) { mPlayPosition[layer].set(val); }
...
void SeqProcessorNotifier::setMuteState(int layer, bool val) { mMuteState[layer].set(val ? 1 : 0); }

void SeqProcessorNotifier::uiNeedsUpdate() { mUINeedsUpdate.set(1); }

void SeqProcessorNotifier::setRandomSeed(int64 seed) { mRandomSeed.set(seed); }
...
SeqProcessorNotifier::PlayRecordState SeqProcessorNotifier::getRecordingState()
{
    return (PlayRecordState)mRecordingState.get();
}
SeqProcessorNotifier::PlayRecordState SeqProcessorNotifier::getPlaybackState()
{
    return (PlayRecordState)mManualPlayingState.get();
}
bool SeqProcessorNotifier::getCompletedMidiNote(int *number, int *velocity, int *len, int *pos)
{
    int third;
...
void SeqProcessorNotifier::clearCompletedMidiNotes() { mCompletedNoteFifo.clearFifo(); }

bool SeqProcessorNotifier::doesUINeedUpdate()
{
...
SequenceData::SequenceData()
    : mSwing(0), mMidiPassthru(SEQ_MIDI_PASSTHRU_NONE), mMidiRespond(SEQCTL_MIDI_RESPOND_YES),
      mMidiMapCount(0), mRandomSeed(0), mOffsetTime(0), mAutoPlay(SEQ_PLAYMODE_AUTO),
...
double SequenceData::getStandaloneBPM() { return mStandaloneBPM; }
void SequenceData::setStandaloneBPM(double bpm)
{
...
SequenceLayer *SequenceData::getLayer(int layer)
{
...
void SequenceData::setGroove(int idx, int val)
{
...
void SequenceData::clearLayer(int layer)
{
    jassert(layer >= 0 && layer <= SEQ_MAX_LAYERS);
    mLayers[layer].clear();
}

void SequenceData::clearPattern(int layer, int pattern)
{
    std::unique_ptr<SequenceLayer::Pattern> blank =
        std::unique_ptr<SequenceLayer::Pattern>(new SequenceLayer::Pattern());
    jassert(layer >= 0 && layer <= SEQ_MAX_LAYERS);
    jassert(pattern >= 0 && pattern <= SEQ_MAX_PATTERNS);
    mLayers[layer].mPats[pattern] = *blank;
}

void SequenceData::clearGroove()
{
    for (int i = 0; i < SEQ_DEFAULT_NUM_STEPS; i++)
        mGroove[i] = 0;
}

void SequenceData::clearMapping()
{
    for (int i = 0; i < SEQMIDI_MAX_ITEMS; i++)
        mMidiMap[i].clear();
}

void SequenceData::setMappingCount(int count)
{
    jassert(count >= 0 && count < SEQMIDI_MAX_ITEMS);
    mMidiMapCount = count;
}

int SequenceData::getMappingCount() { return mMidiMapCount; }

SeqMidiMapItem *SequenceData::getMappingItem(int ind)
{
    jassert(ind >= 0 && ind < SEQMIDI_MAX_ITEMS);
    return &mMidiMap[ind];
}

void SequenceData::setSwing(int val)
{
    jassert(val >= -50 && val <= 50);
    mSwing = val;
}

int SequenceData::getSwing() { return mSwing; }

void SequenceData::setMidiPassthru(int val)
{
    switch (val)
    {
...
int SequenceData::getMidiPassthru() { return mMidiPassthru; }

void SequenceData::setMidiRespond(int val)
{
    switch (val)
    {
...
int SequenceData::getMidiRespond() { return mMidiRespond; }

void SequenceData::copyLayer(int targLayer, int srcLayer)
{
...
void SequenceData::copyScaleData(int targLayer, int srcLayer)
{
    jassert(targLayer >= 0 && targLayer <= SEQ_MAX_LAYERS);
    jassert(srcLayer >= 0 && srcLayer <= SEQ_MAX_LAYERS);
    // mLayers[targLayer] = mLayers[srcLayer];
...
void SequenceData::copyPatternData(int targLayer, int targPat, int srcLayer, int srcPat)
{
    jassert(targLayer >= 0 && targLayer <= SEQ_MAX_LAYERS);
    jassert(srcLayer >= 0 && srcLayer <= SEQ_MAX_LAYERS);
...
int64 SequenceData::getRandomSeed() { return mRandomSeed; }

void SequenceData::setRandomSeed(int64 val) { mRandomSeed = val; }

int SequenceData::getOffsetTime() { return mOffsetTime; }

void SequenceData::setOffsetTime(int ms) { mOffsetTime = ms; }

int SequenceData::getAutoPlayMode() { return mAutoPlay; }

void SequenceData::setAutoPlayMode(int autoplay) { mAutoPlay = autoplay; }

/*============================================
SequenceLayer
*============================================*/
void SequenceLayer::clear()
{
...
void SequenceLayer::setMuted(bool muted) { mMuted = muted; }

bool SequenceLayer::getMuted() { return mMuted; }

bool SequenceLayer::addChainSource(int row, int step, int sourceRow, int sourceStep, bool negtgt,
                                   bool negsrc, int pat)
{
    int i;
    int availableCell = -1;
    ...
    {
        if ((cells[i].flags & SEQ_CHAIN_FLAG_USED) == 0 && availableCell == -1)
        {
            availableCell =
                i; // potentially use this (unless we find a cell representing the same data below)
        }
    ...
    cells[availableCell].flags = SEQ_CHAIN_FLAG_USED | (negtgt ? SEQ_CHAIN_FLAG_NEGTGT : 0) |
...
bool SequenceLayer::getChainSource(int row, int step, int *iterate, int *sourceRow, int *sourceCol,
...
bool SequenceLayer::getChainTarget(int row, int step, int *iterate, int *targRow, int *targCol,
                                   bool *negtgt, bool *negsrc, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
    ...
    for (; i < SEQ_MAX_STEPS; i++)
    {
        ...
        iterate_jump:
            ...
            if ((cells[j].flags & SEQ_CHAIN_FLAG_USED) && cells[j].row == row &&
                cells[j].col == step)
            {
...
void SequenceLayer::setKeyScaleOct(const char *scale, const char *key, int octave)
{
...
void SequenceLayer::getKeyScaleOct(const char **scale, const char **key, int *oct)
{
    *oct = mStdOctave;
    *key = mStdKeyName;
    *scale = mStdScaleName;
...
void SequenceLayer::copyScaleData(const SequenceLayer &src)
{
    int i;
...
int SequenceLayer::getNumSteps() { return mNumSteps; }

void SequenceLayer::setNumSteps(int val)
{
...
void SequenceLayer::setVel(int row, int step, int8_t vel, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
    if (pat == -1)
...
int8_t SequenceLayer::getVel(int row, int step, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
    if (pat == -1)
...
void SequenceLayer::setProb(int row, int step, int8_t prob, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
...
int8_t SequenceLayer::getProb(int row, int step, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
    if (pat == -1)
...
void SequenceLayer::setLength(int row, int step, int8_t length, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
...
int8_t SequenceLayer::getLength(int row, int step, int pat)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    jassert(step >= 0 && step < SEQ_MAX_STEPS);
    if (pat == -1)
...
void SequenceLayer::setNote(int row, int8_t val, bool custom)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
...
int8_t SequenceLayer::getNote(int row, bool custom)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    WhichNoteSet s = custom ? customSet : standardSet;
...
char *SequenceLayer::getNoteName(int row)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    return mNoteSets[customSet].notes[row].noteName;
}

void SequenceLayer::setNoteName(int row, const char *name)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
...
int8_t SequenceLayer::getCurNote(int row)
{
    jassert(row >= 0 && row < SEQ_MAX_ROWS);
    return mNoteSets[mCurrentNoteSet].notes[row].note;
}

int SequenceLayer::getRowForNote(int8_t note)
{
...
void SequenceLayer::setNoteSource(bool custom)
{
...
bool SequenceLayer::noteSourceIsCustom() { return mCurrentNoteSet == customSet; }

int SequenceLayer::getMaxRows() { return mNumRows; }

void SequenceLayer::setMaxRows(int val)
{
    jassert(val >= SEQ_MIN_ROWS && val <= SEQ_MAX_ROWS);
    mNumRows = val;
}

int SequenceLayer::getMaxPoly() { return mMaxPoly; }

void SequenceLayer::setMaxPoly(int val)
{
    jassert(val >= 1 && val <= SEQ_MAX_ROWS);
    mMaxPoly = val;
}

int SequenceLayer::getPolyBias() { return mPolyBias; }

void SequenceLayer::setPolyBias(int val)
{
    jassert(val >= SEQ_POLY_BIAS_MIN && val <= SEQ_POLY_BIAS_MAX);
    mPolyBias = val;
}

bool SequenceLayer::isMonoMode() { return mIsMonoMode; }

void SequenceLayer::setMonoMode(bool val) { mIsMonoMode = val; }

bool SequenceLayer::isCombineMode() { return mCombineMode; }

void SequenceLayer::setCombineMode(bool val) { mCombineMode = val; }

int SequenceLayer::getClockDivider() { return mClockDivider; }

void SequenceLayer::setClockDivider(int c)
{
    jassert(c >= SEQ_MIN_CLOCK_DIV && c <= SEQ_MAX_CLOCK_DIV);
    mClockDivider = c;
}

int8_t SequenceLayer::getMidiChannel() { return mMidiChannel; }

void SequenceLayer::setMidiChannel(int8_t val)
{
    jassert(val >= 1 && val <= 16);
    mMidiChannel = val;
}

int SequenceLayer::getStepsPerMeasure() { return mStepsPerMeasure; }

void SequenceLayer::setStepsPerMeasure(int val)
{
    jassert(val >= SEQ_MIN_STEPS && val <= SEQ_DEFAULT_NUM_STEPS);
    mStepsPerMeasure = val;
}

int SequenceLayer::getDutyCycle() { return mDutyCycle; }

void SequenceLayer::setDutyCycle(int val)
{
...
int SequenceLayer::getHumanVelocity() { return mHumanVelo; }
...
int SequenceLayer::getHumanPosition() { return mHumanPos; }
...
int SequenceLayer::getHumanLength() { return mHumanLen; }

const char *SequenceLayer::getLayerName() { return mLayerName; }
...
int SequenceLayer::getCurrentPattern() { return mCurrentPattern; }
