...
void StepCpt::paint(Graphics &g)
{
    ...
    if (getEffectiveColorAndText(c, txt))
    {
        ...
        if (marker)
        { // check to see if it's a boundary step that needs special attention
            if (marker == 1)
            { // start of measure
...
bool StepCpt::getEffectiveColorAndText(juce::Colour &c, juce::String &txt, int layer)
{
    EditorState *e = mGlob->mEditorState;
    SeqDataBuffer *s = mGlob->mSeqBuf;
...
StepPanel::StepPanel(SeqGlob *glob, int id, Component *mainCpt, CptNotify *notify, String name)
    : Component(name), mMainEditor(mainCpt), mGlob(glob), mId(id), mNotify(notify),
      mLengthEditCursor(), mSelGridItem(-1), mLastLayer(-1), mCurLayer(0), // first layer
      mAxis(unknown), mMouseStartVal(MOUSE_STARTVAL_INVALID), mChainStartItem(0), mChainEndItem(0),
      mChainCustom(false), mChainNegTgt(false), mRowNotify(0), mDoingMultiSelect(false),
      mChordHandler(glob), mCurPosition(-1)
...
static inline int8_t getNewMonoVal(int8_t curVal, int delta)
{
    static int vals[] = {SEQ_PROB_OFF, SEQ_PROB_NEVER, SEQ_PROB_LOW_VAL, SEQ_PROB_MED_VAL,
                         SEQ_PROB_HIGH_VAL};
...
void StepPanel::paintOverChildren(Graphics &g)
{
    ...
    if (mGlob->mEditorState->getEditMode() == EditorState::editingSteps && mChordHandler.inCell())
    {
        ...
        g.setColour(mGlob->mEditorState->getColorFor(EditorState::coloredElements::cellSelection)
                        .withAlpha(.6f));
        g.fillRect(b.toFloat());
        g.setColour(mGlob->mEditorState->getColorFor(EditorState::coloredElements::cellSelection)
                        .withAlpha(.2f));
...
void StepPanel::mouseDrag(const MouseEvent &event)
{
    ...
    else
    {
        ...
        { // multiselect mode is active
            ...
            {
                // change state of multi selection (multi selection may not be normalized after
                // this)
            ...
            if (mMouseStartVal != MOUSE_STARTVAL_INVALID)
            {
                ...
                if (mouseDist)
                {
                    ...
                    else if (e->getEditMode() == EditorState::editingTriggers)
                    {
                        ...
                        newval = mMouseStartVal + mouseDist;
                        newval = newval > SEQ_MAX_RETRIGGER ? SEQ_MAX_RETRIGGER
                                 : newval < 1               ? 1
                                                            : newval;
                    }
                    else if (data->isMonoMode())
                    {
                        // since there are limited possible values in monoMode, make the mouse less
                        // sensitive
                        mouseDist /= 5;
                        // there are 4 possible values so the max we'll move from current val is 3
                        // (or -3)
                    ...
                    { // poly mode
                      // clamp to 100
                        ...
                        newval = newval > SEQ_PROB_ON    ? SEQ_PROB_ON
                                 : newval < SEQ_PROB_OFF ? SEQ_PROB_OFF
                                                         : newval;
                    }

                    // write to a temporary value on the step itself. This will be written to seq on
                    // mouseUp also used during paint
                    c->mTempValue = (int8_t)newval;
                }
                repaint();
            }
            else
            { // mouse start val not -1
                if (mChainStartItem)
                {
...
void StepPanel::mouseDown(const MouseEvent &event)
{
    ...
    else if (event.eventComponent->getName().compare("singleStep") == 0)
    {
        ...
        if (e->getEditMode() == EditorState::editingVelocity)
        {
            if (data->getProb(c->mRow, c->mCol) !=
                SEQ_PROB_OFF) // only allow velocity edit on "on" notes
                mMouseStartVal = data->getVel(c->mRow, c->mCol);
        }
        else if (e->getEditMode() == EditorState::editingOffset)
        {
            if (data->getProb(c->mRow, c->mCol) !=
                SEQ_PROB_OFF) // only allow offset edit on "on" notes
        ...
        else if (e->getEditMode() == EditorState::editingSteps)
        {
            ...
            // (that's all) TODO do this always if selsize==1 rather otherwise it doesn't work as
            // desired when shifted
...
void StepPanel::mouseUp(const MouseEvent &event)
{
    ...
    else if (cn == "singleStep")
    {
        ...
        if (mChainStartItem)
        { // if the mouseDown was on an item and we are in chain mode
            if (mChainStartItem == mChainEndItem || mChainEndItem == 0)
            {
                if (rightMouse)
                {
                    ...
                    else if (data->getProb(mChainStartItem->mRow, mChainStartItem->mCol) ==
                             SEQ_PROB_NEVER)
                    {
            ...
            else if (mChainEndItem)
            { // two different items, add a link
                ...
                if (mChainEndItem->mCol < data->getNumSteps())
                {
                    if (data->getProb(mChainEndItem->mRow, mChainEndItem->mCol) == SEQ_PROB_OFF)
                    {
                        ...
                        data->setVel(mChainEndItem->mRow, mChainEndItem->mCol,
                                     e->getDefaultVelocity());
                        data->setLength(mChainEndItem->mRow, mChainEndItem->mCol,
                                        0); // default length
                    ...
                    if (data->addChainSource(mChainEndItem->mRow, mChainEndItem->mCol,
                                             mChainStartItem->mRow, mChainStartItem->mCol,
                                             mChainNegTgt, false))
                    {
                        buf->swap();
                        c = mChainEndItem; // so that this items ends up being selected (below where
                                           // we select item)
                        ...
                        if (mChainCustom)
                        {
                            String f = String::formatted(
                                "chainAdd|%d|%d|%d|%d", mChainStartItem->mRow,
                    ...
                    else
                    {
                        ...
                        String txt = String::formatted(
                            "Maximum number of chains (%d) for this column has been reached",
                            SEQ_MAX_CHAIN_SOURCES);
        ...
        oldTrig = newTrig = t;
        mAxis = unknown;
        if (mMouseStartVal != MOUSE_STARTVAL_INVALID && e->getNumSelectedCells() < 2)
        { // if we are interacting with a valid cell
...
void StepPanel::resized()
{
    ...
    for (int x = 0; x < SEQ_MAX_STEPS; ++x)
    {
        for (int y = 0; y < SEQ_MAX_ROWS; ++y)
        {
            ...
            Rectangle<int> elementBounds(x * stepX, (y * SEQ_SIZE_CELL_H) - invisibleHeight, stepX,
                                         SEQ_SIZE_CELL_H);
...
void StepPanel::paintGhostedLayer(Graphics &g)
{
    ...
    */
    ...
    for (int i = 0; i < SEQ_MAX_ROWS * SEQ_MAX_STEPS; i++)
    {
        ...
        if (cpt->isStepInRange(mLastLayer) && cpt->isStepInRange() && // is it visible?
            data->getProb(cpt->mRow, cpt->mCol) != SEQ_PROB_OFF)
        { // is it on?
                      ...
                      Colour lineColor, Colour blobColor)
{
    ...
    PathStrokeType stroke(thickness, PathStrokeType::JointStyle::curved,
    ...
    // draw the arrow head
    stroke = PathStrokeType(thickness * 2.0f / 3.0f, PathStrokeType::JointStyle::mitered,
                            PathStrokeType::EndCapStyle::rounded);
...
void StepPanel::paintChains(Graphics &g)
{
    ...
    if (chEnd->isStepInRange())
    {
        ...
        while (data->getChainSource(y, x, &iterate, &sy, &sx, &negtgt, &negsrc))
        {
            ...
            if (chStart->isStepInRange())
            {
                ...
                if (current)
                { // chains leading to currently selected cell
                    if (negtgt)
                        selColor = negSel;
                    else
                        selColor = posSel;
...
void StepPanel::paintSelectedCells(Graphics &g)
{
    EditorState &e = *mGlob->mEditorState;
    int startX, endX, startY, endY;
    ...
    if (e.getSelectedCells(&startX, &endX, &startY, &endY))
    {
        juce::Colour overlay = e.getColorFor(EditorState::cellSelection).withAlpha(0.6f);
        StepCpt *cpttl, *cptbr;
...
void StepPanel::paintLengthBars(Graphics &g)
{
    ...
    */
    ...
    for (int i = 0; i < (SEQ_MAX_ROWS - 1) * SEQ_MAX_STEPS; i++)
    {
        ...
        { // is it on?
            ...
            if (wrapped)
            { // length that wrapped around to beginning
                lenRect = b;
                lenRect.removeFromTop(lenRect.getHeight() / 2);
                // move to end of cell
                lenRect.setX(0);
            ...
            if (i == mSelGridItem)
            {
                ...
                int wrapSelected = 0;
                if (steplen > 0)
                { // steplen will be 0 or a positive value
                    if (steplen > maxBeforeWrap)
                    {
...
void StepPanel::setRowNotify(StepRowNotify *notify) { mRowNotify = notify; }

void StepPanel::moveLengthEditCursor()
{
    ...
    if (mSelGridItem != -1)
    {
        ...
        if (c->mCol < data->getNumSteps() && data->getProb(c->mRow, c->mCol) != SEQ_PROB_OFF &&
            data->getCurNote(c->mRow) != -1)
        { // don't allow length edit on "off" row
...
============================================================================================*/
void SeqChordHandler::recalcNotes()
{
    SequenceLayer *lay =
        mGlob->mSeqBuf->getUISeqData()->getLayer(mGlob->mEditorState->getCurrentLayer());
    int row;
    int i;
    int noteNum;
...
void SeqChordHandler::addInterval(int semitones)
{
...
int SeqChordHandler::numUsedIntervals() { return mUsed; }

void SeqChordHandler::leaveCell() { mInCell = false; }
...
int SeqChordHandler::getCol() { return mCol; }

int SeqChordHandler::getRootRow() { return mRootRow; }
