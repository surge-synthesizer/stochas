...
class EditorState
{
  public:
    ...
    // given current velocity, returns next velocity value that would be needed when mouse is
    // clicked if next is false it cycles backwards
    ...
    // mouse sensitivity for dragging stuff. 1..SEQ_MOUSE_SENSE_MAX where 1 is highest and n is
    // lower
    int getMouseSense();
  ...
  private:
    ...
    // read/write individual int setting (min max ensure that values read from file are not out of
    // range) def is default value
    void configGetSetInt(juce::PropertiesFile *pf, bool read, int &val, const char *key, int min,
                         int max, int def);

    void configGetSetString(juce::PropertiesFile *pf, bool read, String &val, const char *key,
...
class SeqGlob
{
  public:
    ...
    SeqGlob(SeqDataBuffer *d, EditorState *es, SeqProcessorNotifier *n, SeqFifo *fif,
            SeqAudioProcessor *p)
    ...
    ~SeqGlob() { mSeqBuf->setChangeNotify(0, 0); }
};
