...
rangeLo-1 = SeqAudioProcessorParameter's idea of default (because we need a smooth range where it
occurs at the bottom) SEQ_AUT_DEFAULT_VALUE_DESIG = SeqPluginProcessor's idea of default (because we
need something clearly outside of any valid range)
...
class SeqAudioProcessorParameter : public AudioProcessorParameter
{
  ...
  public:
    SeqAudioProcessorParameter(AutParamNotify *notify, int paramId, int rangeLo, int rangeHi,
                               int layerNum, const String &name);
...
// TODO playback automation: in reaper at least, if you add a knob for this parameter and set it to
// "stop", then go into the stochas ui and hit play button it ignores this parameter being in "stop"
...
/**
 */
class SeqAudioProcessor : public AudioProcessor,
                          public SeqProcessorNotifierHelper,
                          public AutParamNotify
{
    ...
    inline bool determinePlaybackState(int apm, bool playingInDaw, double beatsperbar,
                                       double samplerate, double bpm, double beatPosition,
                                       int samplesperblock);
    ...
    void handleIncomingMidi(bool currentlyPlaying, bool startingPlayback, MidiBuffer &midiMessages,
