...
PlayPanel::PlayPanel(SeqGlob *glob) : Component("playPanel"), mGlob(glob), mCurPosition(-1)
{
...
PlayLightCpt::PlayLightCpt(const String &name) : Component(name), mOn(false) {}
...
void PlayLightCpt::paint(Graphics &g)
{
    ...
    if (getWidth() > 20)
    { // todo: this is approximation. figure out better way. is there a function to determine?
...
PatternPlayPanel::PatternPlayPanel(SeqGlob *glob)
    : Component("patPanel"), mGlob(glob), mCurPosition(-1)
{
