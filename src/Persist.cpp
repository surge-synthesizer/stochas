...
void SeqPersist::retrieveLayer(XmlElement *e, SequenceLayer *lay)
{
    ...
    forEachXmlChildElement(*e, ch)
    {
        ...
        {
            forEachXmlChildElementWithTagName(*ch, pat, "p") { retrievePattern(pat, lay); }
        }
            ...
            if (rowidx >= 0 && rowidx < SEQ_MAX_ROWS)
            {
                forEachXmlChildElementWithTagName(*row, cells, "cells")
                {
                    forEachXmlChildElementWithTagName(*cells, cell, "c")
                    {
                        ...
                        {
                            lay->setProb(rowidx, cellidx,
                                         (int8_t)cell->getIntAttribute("prob", SEQ_PROB_OFF), pat);
                            lay->setVel(rowidx, cellidx, (int8_t)cell->getIntAttribute("velo"),
                                        pat);
                            lay->setLength(rowidx, cellidx, (int8_t)cell->getIntAttribute("len"),
                                           pat);
                            lay->setOffset(rowidx, cellidx, (int8_t)cell->getIntAttribute("offs"),
                                           pat);
                            ...
                            forEachXmlChildElementWithTagName(*cell, cs, "cs")
                            {
                                ...
                                lay->addChainSource(rowidx, cellidx, srcRow, srcStep, negtgt,
                                                    negsrc, pat);
                            }
                        }
                    }
...
const XmlElement &SeqPersist::store(SequenceData *sourceData)
{
    ...
    */
    ...
    mRoot.prependChildElement(
        addKeyVal("offtime", (int64)sourceData->getOffsetTime())); // global offset time
