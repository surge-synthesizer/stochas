...
class SeqFifo
{
  public:
    ...
    void clearFifo() { abstractFifo.reset(); }
...
class SequenceLayer
{
    ...
    struct Note
    {
        ...
        Note() : note(0) { memset(noteName, 0, SEQ_MAX_NOTELABEL_LEN); }
    };
    ...
    // note that if length of a cell is > 0, then this cycle applies to the last cell-length that
    // plays
    int mDutyCycle;
  ...
  public:
    ...
    bool addChainSource(int row, int step, int sourceRow, int sourceStep, bool negtgt, bool negsrc,
                        int pat = -1);
    ...
    bool getChainSource(int row, int step, int *iterate, int *sourceRow, int *sourceCol,
                        bool *negtgt, bool *negsrc, int pat = -1);
    bool getChainTarget(int row, int step, int *iterate, int *targRow, int *targCol, bool *negtgt,
                        bool *negsrc, int pat = -1);
...
class SeqDataBuffer
{
  ...
  public:
    ...
    SequenceData *getAudSeqData() { return &mBuffer[mCurrent.get()]; }
    ...
    SequenceData *getUISeqData() { return &mBuffer[mCurrent.get() ? 0 : 1]; }
};
...
class SeqProcessorNotifier
{
  ...
  public:
    ...
    //
    bool getCompletedMidiNote(int *number, int *velocity, int *len /*in steps*/,
                              int *pos /*on grid*/);
    ...
    void addCompletedMidiNote(int number, int velocity, int len /*in steps*/,
                              int pos /*on the grid*/);
