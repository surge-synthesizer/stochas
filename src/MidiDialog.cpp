    ...
    : SeqModalDialog(glob, 666, parent, 600, 350), mGlob(glob), mLearningRow(-1)
{
    ...
    mTable->setColour(ListBox::outlineColourId,
                      mGlob->mEditorState->getColorFor(EditorState::border));
    ...
    mBtnReset =
        std::unique_ptr<TextButton>(addStdButton("Reset to Default", nullptr, SEQCTL_MIDI_RESET));
    mBtnAdd = std::unique_ptr<TextButton>(addStdButton("Add New", nullptr, SEQCTL_MIDI_ADD));
}

SeqMidiDialog::~SeqMidiDialog() {}

int SeqMidiDialog::getNumRows() { return mMapping.size(); }
...
void SeqMidiDialog::midiMsgReceived(int8_t type, int8_t chan, int8_t num, int8_t /*val*/)
{
    ...
    if (mLearningRow != -1)
    {
        ...
        if ((mi.mAction == SEQMIDI_ACTION_PBIAS && type == SEQ_MIDI_CC) ||
            (mi.mAction != SEQMIDI_ACTION_PBIAS &&
             (type == SEQ_MIDI_NOTEON || type == SEQ_MIDI_NOTEOFF)))
        {
...
SeqMidiRow::SeqMidiRow(SeqGlob *glob, SeqMidiDialog *dlg)
    : mGlob(glob), mDlg(dlg), mRowNumber(-1),
...
void SeqMidiRow::buttonClicked(Button *btn)
{
...
void SeqMidiRow::cptValueChange(int cptId, int value)
{
...
void SeqMidiRow::fillValueListBasedOnAction()
{
    ...
    case SEQMIDI_ACTION_STEPS:
        ...
        mCBValue.addItem(String::formatted("%d Steps", SEQMIDI_VALUE_NS_TWENTYFOUR),
                         SEQMIDI_VALUE_NS_TWENTYFOUR);
        mCBValue.addItem(String::formatted("%d Steps", SEQMIDI_VALUE_NS_TWO), SEQMIDI_VALUE_NS_TWO);
        mCBValue.addItem(String::formatted("%d Steps", SEQMIDI_VALUE_NS_THREE),
                         SEQMIDI_VALUE_NS_THREE);
        mCBValue.addItem(String::formatted("%d Steps", SEQMIDI_VALUE_NS_FOUR),
                         SEQMIDI_VALUE_NS_FOUR);
...
void SeqMidiRow::setRowNumber(int val)
{
    ...
    // in case color scheme is changing (via refreshComponentForRow, via updateContent in fixColors
    // common cpt)
    setComboColor(&mCBAction);
    setComboColor(&mCBTarget);
    setComboColor(&mCBValue);
...
void SeqMidiRow::setComboColor(ComboBox *cb)
{
...
bool SeqMidiRow::getNumberCptCustomText(int id, int value, String &repl)
{
    ...
    else
    {
        ...
        repl = SeqScale::getMidiNoteName((char)value, lowOct, buf,
                                         mGlob->mEditorState->isShowMidiNumbers());
        repl << suff;
    }
    return true;
}
