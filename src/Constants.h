...
#define SEQ_RESIZE_MSG                                                                             \
    "UI was resized. Depending on your host application, you might need to restart Stochas to "    \
    "properly adjust the window size"

// add some code to get around cubase behavior where it actually starts play position prior to
// where the cursor is when user hits play. if any issues arise, disable this to see if the
// workaround code is the cause. its a hack because it assumes that we will get process block called
...
#define SEQ_MAX_NOTELABEL_LEN                                                                      \
    16 // maximum custom name (label, not actual note) for a note (excl terminator)
...
#define SEQ_SIZE_MAIN_W                                                                            \
    ((SEQ_DEFAULT_NUM_STEPS * SEQ_SIZE_CELL_W) + SEQ_SIZE_NOTE_W + (SEQ_SIZE_MAIN_BORDER * 2) +    \
     SEQ_SIZE_VSCROLL)
...
#define SEQCTL_OPTION_PANEL                                                                        \
    6 //* id of the option panel for the purpose of notifying main ui to repaint
...
// midi mapping
// transpose. the ordering is: -11..-1 semi, +1..+11 semi, -2..-1 oct, 1..2 oct, next semi, prev
// semi
...
#define SEQ_NOTIFY_HOST                                                                            \
    101 // notify host that a change has occurred so that it knows the state is dirty
#define SEQ_SET_RECORD_MODE 102 // notify processor thread that recording is on/off (toggle)
#define SEQ_SET_PLAY_START_STOP                                                                    \
    103 // notify processor that manual playback is started (1) or stopped (0)
