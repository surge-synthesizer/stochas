...
ToggleCpt::ToggleCpt(SeqGlob *glob, int id, CptNotify *notify, String name)
    : Component(name), mId(id), mAlwaysNotify(false), mNotify(notify), mItemsPerRow(0xFFFF),
      mGlob(glob)
{
}
...
void ToggleCpt::setCurrentItem(int id, bool val, bool triggerNotify)
{
    ...
    else
    {
        ...
        for (int i = 0; i < mItems.size(); i++)
        {
            ...
            {
                if (tmp.selected != val)
                { // if this item is changing value
                    doNotify = true;
                    tmp.selected = val;
                }
                notifyVal = tmp.id;
...
void ToggleCpt::setAlwaysNotify(bool val) { mAlwaysNotify = val; }
...
ButtonCpt::ButtonCpt(SeqGlob *glob, int id, CptNotify *notify, String name)
    : Component(name), mId(id), mGlob(glob), mNotify(notify)
{
}
...
void ButtonCpt::overrideColor(juce::Colour c) { mOverrideColor = c; }

/*===========================================================================================
NumberCpt implementations
============================================================================================*/
NumberCpt::NumberCpt(SeqGlob *glob, int id, CptNotify *notify, String name)
    : Component(name), mGlob(glob), mId(id), mValue(0), mDefaultValue(0), mInterval(1),
      mLowBound(0), mHighBound(0), mNotify(notify), mInteracting(false), mDragValue(0),
      mIntSense(1), mAxis(unknown), mEnabled(true), mTextEdit(0), mTextReplacer(0)
{
}
...
String NumberCpt::getTextualValue(bool stripped)
{
    ...
    else if (mReplacements.contains(val))
    {
...
void NumberCpt::enable(bool en) { mEnabled = en; }
...
void NumberCpt::setStringRep(int val, String repl) { mReplacements.set(val, repl); }

void NumberCpt::setCustomText(CustomText *replacer) { mTextReplacer = replacer; }

int NumberCpt::getValue() { return mValue; }
...
void GrooveCpt::paint(Graphics &g)
{
    ...
    for (i = 0; i < (SEQ_DEFAULT_NUM_STEPS * 2); i++)
    {
        ...
        { // main line
            ...
            g.drawText(String::formatted("%d", (i / 2) + 1), txtRect, Justification::centred,
                       false);
            ...
            if (val && mEnabled)
            {
                ...
                if (width < 0)
                    width *= -1;
                Rectangle<float> box;
                box.setBounds(ln.getStartX(), ln.getStartY(), width, h.getHeight() / 2);
...
TabPanelCpt::TabPanelCpt(SeqGlob *glob, bool gradientFill) : mGlob(glob), mGradient(gradientFill) {}

void TabPanelCpt::paint(Graphics &g)
{
    if (mGradient)
...
void SeqModalDialog::forceResize() { resizedInner(&mPanel); }

void SeqModalDialog::notifyParent(int val) { mParent->cptValueChange(mId, val); }

ToggleButton *SeqModalDialog::addToggle(const String &txt, int grp, Component *parent, int cptId)
{
    if (!parent)
        parent = &mPanel;
...
ComboBox *SeqModalDialog::addCombo(const String &txt, Component *parent, int cptId)
{
...
Label *SeqModalDialog::addStdLabel(const String &txt, Component *parent)
{
    if (!parent)
        parent = &mPanel;
...
void SeqModalDialog::removeFromInner(Component *cpt) { mPanel.removeChildComponent(cpt); }
...
void SeqModalDialog::cptValueChange(int cptId, int value) { notify(cptId, value); }

void SeqModalDialog::buttonClicked(Button *b) { notify(b->getComponentID().getIntValue(), 0); }
...
void SeqModalDialog::InnerPanel::resized() { mParent->resizedInner(this); }
