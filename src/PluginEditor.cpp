...
/* Given a number of steps, return number of pages needed to accommodate that
 number of steps
 */
static int numStepPages(int steps)
{
...
SeqAudioProcessorEditor::SeqAudioProcessorEditor(SeqAudioProcessor &p)
    : AudioProcessorEditor(&p), mProcessor(p),
      ...
      mMidiIndicator("midiIndicator"), mPlayPanel(&mGlob), // play position indicator
      mPatPlayPanel(&mGlob),                               // pattern play panel
      mEditToggle(&mGlob, SEQCTL_EDIT_TOGGLE, this,
                  "editModeSelect"), // which mode we are looking at: prob/velo/opt
      ...
      mSectionSelect(&mGlob, SEQCTL_SECTION_TOGGLE, this,
                     "sectionSelect"), // select section when number of steps exceeds 16
      mMainTabs(SEQCTL_TABS, this,
                TabbedButtonBar::Orientation::TabsAtBottom), // main tabs at bottom
      ...
      mInfoDialog(&mGlob, this), mFileChooser(&mGlob, this), mChainDialog(&mGlob, this),
      ...
      mMidiDlg(&mGlob, this)
{
    ...
    mPatternSelect.setCurrentItem(
        mGlob.mSeqBuf->getUISeqData()->getLayer(curlyr)->getCurrentPattern() + 1, true, false);
    ...
    mLblChords.setText("Note: Use up/down arrow keys to set chord inversions",
...
void SeqAudioProcessorEditor::resized()
{
    ...
    Rectangle<int> gridctrl, lower, left, upper, tmp, tmp2, options, midiInd, patToggle, banner,
        scrl, scrlH, pph;
    ...
    lower = me.removeFromBottom(me.getHeight() - ((SEQ_SIZE_CELL_H * SEQ_MAX_VISIBLE_ROWS) +
                                                  SEQ_SIZE_PP_H + SEQ_SIZE_HSCROLL));
...
void SeqAudioProcessorEditor::setStepRangeVisible()
{
    ...
    { // show all steps (scale the width appropriately)
        ...
        //   // Where on the scrollbar should the thumb be, given number of pages and which page we
        //   are on thumb = (double)p / (double)numStepPages(sl->getNumSteps());
    }
    else
    {
        ...
        mPlayPanel.setBounds(-(ls * SEQ_SIZE_CELL_W), 0, SEQ_SIZE_CELL_W * SEQ_MAX_STEPS,
                             SEQ_SIZE_PP_H);

        mStepPanel.setBounds(-(ls * SEQ_SIZE_CELL_W), 0, SEQ_SIZE_CELL_W * SEQ_MAX_STEPS,
    ...
    mNotePanel.setBounds(0, 0, mNoteHolder.getWidth(), sl->getMaxRows() * SEQ_SIZE_CELL_H);
...
void SeqAudioProcessorEditor::cptValueChange(int cptId, int id)
{
    ...
    case SEQCTL_SETTINGS_TAB:
        // one of the items on the settings tab changed. id will be the id on that tab of the item
        // that changed
        switch (id)
        {
        case SEQCTL_SET_UISCALE:
            resized();
            // this is cheezy I know, but I am not sure what hosts will not resize themselves
            // properly
    ...
    case SEQCTL_CHORD_TOGGLE:
        chordSelect(id);
        break;
    case SEQCTL_FILEDIALOG: // file dialog had "OK" hit on it, access members on it to determine
                            // what to do
        respondFileChooser();
        repaint();
        break;
...
void SeqAudioProcessorEditor::loadMidiGroove(const String &filename)
{
    ...
    {
        setAlertText("Failed to read data from file. Possibly it is not a MIDI file, or it has no "
                     "valid tracks.");
        return;
    }
...
void SeqAudioProcessorEditor::checkForRecordedNotes()
{
    ...
    while (
        mGlob.mAudNotify->getCompletedMidiNote(&midiRecNum, &midiRecVel, &midiRecLen, &midiRecPos))
    {
...
void SeqAudioProcessorEditor::updateUI()
{
    ...
    mMainTabs.setTabName(
        0, String::formatted("Layer %d Options", mGlob.mEditorState->getCurrentLayer() + 1));
