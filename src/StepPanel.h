...
class StepCpt : public Component
{
  public:
    ...
    // returns true if step is visible in current view (within row and column bounds, step page,
    // etc)
...
class StepPanel : public Component, public KeyListener
{
    ...
    // if xaxis is false, we are shifting on y axis, if positive is false we are shifting left/up
    // (depending on axis)
  ...
  public:
    ...
    StepPanel(SeqGlob *glob, int id, Component *mainCpt, CptNotify *notify,
              String name = "undefined" /* will be set later*/);
