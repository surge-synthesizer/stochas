...
NoteCpt::NoteCpt(String name)
    : Component(name), mParent(0), mBtnPlay(this), mTempValue(-1), mTextEdit(0)
{
...
void NoteCpt::paint(Graphics &g)
{
    ...
    // midi note name
    const char *nn =
        SeqScale::getMidiNoteName(n, e->getLowestOctave(), noteBuf, e->isShowMidiNumbers());
    g.drawText(nn, r.toFloat(), locked ? Justification::centred : Justification::right);
}
...
void NoteCpt::mouseUp(const MouseEvent &event)
{
    ...
    {
        mGlob->mProcessNotify->addToFifo(SEQ_MIDI_NOTEOFF, mGlob->mEditorState->getCurrentLayer(),
                                         mId);
    }
}
...
void NoteCpt::mouseDown(const MouseEvent &event)
{
    if (event.eventComponent == &mBtnPlay)
    {
        ...
        */
        mGlob->mProcessNotify->addToFifo(SEQ_MIDI_NOTEON, mGlob->mEditorState->getCurrentLayer(),
                                         mId);
    }
}
...
============================================================================================*/
NotePanel::NotePanel(SeqGlob *glob)
    : Component("notePanel"), mGlob(glob), mAxis(unknown), mMouseStartVal(-1), mMouseOverRow(-1)
{
...
void NotePanel::resized()
{
    ...
    for (int y = 0; y < SEQ_MAX_ROWS; ++y)
    {
        ...
        Rectangle<int> elementBounds(0, (y * SEQ_SIZE_CELL_H) - invisibleHeight, width,
                                     SEQ_SIZE_CELL_H);
