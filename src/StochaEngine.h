...
class StochaEngine
{
    ...
    /* represent a single mapping and what it does.
       A mapping is a user defined value that says "when i hit note A4 I want to change pattern in
       layer x"
    ...
    */
    struct MidiMappingItem
    {
        ...
        MidiMappingItem()
            : mChannel(0), mAction(SEQMIDI_ACTION_INVALID), mValue(0), mType(0), mNext(nullptr)
        {
        }
    };
    ...
    struct MidiOverride
    {
        ...
        inline void override(int newval)
        {
            mValue = newval;
            mOverriden = true;
        }
    ...
    */
    struct StochaEvent
    {
        ...
        StochaEvent
            *mCorrespondingNoteOff; // if this is a note on event, this will point at it's note off
        StochaEvent() { clear(); }
        void clear()
        {
            mNumSamples = -1;
    ...
    // this is the step position that is used for visually informing the user what position is
    // current
    ...
    */
    ...
    /*
    This is called some time before a grid step occurs (could be up to half a step before it
    occurs). samples_per_step - how many samples are in each grid step position - the position in
    the sequence (grid step) that is about to occur (in steps) samples_until - number of samples
    before the grid step occurs returns false if one or more midi notes that needed to be inserted
    ...
    */
    ...
    /*
    Generate multiple notes. Returns number of notes to play and mMulti is filled with row numbers
    of these notes. will not produce more than maxpoly
    */
  ...
  public:
    ...
    in the pattern. it may also return -1 if there is no current step position (eg playback
    stopped). This is used to inform the user where we are. If fraction is specified it returns a
    value between 0 and 99 indicating what % of the way thru the step we are (for midi record)
    */
    int getCurrentStepPosition(int *fraction = 0);

    /* this returns step position in the whole track (ie doesn't flip back to 0 when numsteps is
     * reached */
    ...
    */
    ...
    on the midi mapping, and having received midi data that matches that mapping. this should be
    called whenever playing stops.
    */
