...
EditorState::EditorState()
    : mEditMode(editingSteps), mCurrentLayer(0), mVisibleStep(0), mSelectedCells(0),
      mSelectedLowRow(0), mSelectedHighRow(0), mSelectedLowCol(0), mSelectedHighCol(0)
...
EditorState::EditMode EditorState::getEditMode() { return mEditMode; }

void EditorState::setEditMode(EditMode newMode) { mEditMode = newMode; }
...
int EditorState::getVisibleStep() { return mVisibleStep; }
...
int8_t EditorState::probCycleNext(int8_t curprob, bool mono, bool next)
{
    ...
    { // cycle to next
        ...
        { // prob=high val and we are in poly mode
...
//===================================================================================
void EditorState::configGetSetInt(juce::PropertiesFile *pf, bool read, int &val, const char *key,
...
void EditorState::configGetSetString(juce::PropertiesFile *pf, bool read, String &val,
                                     const char *key, const String &def)
{
    if (read)
    {
...
EditorState::mouseRightClickAction EditorState::getMouseRightClickAction()
{
...
{
    if (lowCol)
        *lowCol = mSelectedLowCol;
    if (hiCol)
        *hiCol = mSelectedHighCol;
    if (lowRow)
        *lowRow = mSelectedLowRow;
    if (hiRow)
        *hiRow = mSelectedHighRow;
...
void EditorState::clearSelectedCells() { mSelectedCells = false; }
...
bool EditorState::isCellSelected(int col, int row)
{
    if (!mSelectedCells)
        return false;
    if (col >= mSelectedLowCol && col <= mSelectedHighCol && row >= mSelectedLowRow &&
        row <= mSelectedHighRow)
        return true;
    return false;
}

bool EditorState::isShiftReversed() { return mShiftReversed == 1; }

void EditorState::setShiftReversed(bool isReversed) { mShiftReversed = isReversed ? 1 : 0; }

bool EditorState::isPatLayerLinked() { return mPatLayerLinked == 1; }

void EditorState::setPatLayerLinked(bool isLinked) { mPatLayerLinked = isLinked ? 1 : 0; }

void EditorState::setShowMidiNumbers(bool useNumbers) { mShowMidiNumbers = useNumbers ? 1 : 0; }
bool EditorState::isShowMidiNumbers() { return mShowMidiNumbers == 1; }

void EditorState::setFileDirectory(const String &dir) { mDefaultFileDir = dir; }

String EditorState::getFileDirectory() { return mDefaultFileDir; }

bool EditorState::getKeyboardDisabled() { return mKeyboardDisabled == 1; }

void EditorState::setPPQOffset(int offset) { mPPQOffset = offset; }

int EditorState::getPPQOffset() { return mPPQOffset; }

void EditorState::setScaleFactor(int factor) { mScaleFactor = factor; }

int EditorState::getScaleFactor() { return mScaleFactor; }
...
void EditorState::configSerialization(bool read)
{
    ...
    configGetSetInt(pf, read, mMouseSense, "mouseSense", 1, SEQ_MOUSE_SENSE_MAX,
                    SEQ_MOUSE_SENSE_DEFAULT);
    configGetSetInt(pf, read, mDefaultVelocity, "defaultVelocity", 0, 127, SEQ_VELOCITY_DEFAULT);
    configGetSetInt(pf, read, mLowestOctave, "lowestOctave", SEQ_BASE_OCT_LOW, SEQ_BASE_OCT_HIGH,
                    SEQ_BASE_OCT_DEFAULT);
    configGetSetInt(pf, read, mDefaultMono, "defaultMono", SEQ_PROB_OFF, SEQ_PROB_ON,
                    SEQ_PROB_DEFAULT_MONO);
    configGetSetInt(pf, read, mDefaultPoly, "defaultPoly", SEQ_PROB_OFF, SEQ_PROB_ON,
                    SEQ_PROB_DEFAULT_POLY);
    ...
    if (hostType.isProTools())
        ...
        tmp = 0;
    configGetSetInt(pf, read, mPPQOffset, "positionOffset", SEQ_POS_OFFSET_MIN, SEQ_POS_OFFSET_MAX,
                    tmp);

    configGetSetInt(pf, read, mScaleFactor, "uiScaleFactor", SEQ_UI_SCALE_MIN, SEQ_UI_SCALE_MAX,
                    SEQ_UI_SCALE_DEFAULT);
...
int EditorState::getMouseSense() { return mMouseSense; }
...
int8_t EditorState::getDefaultVelocity() { return (int8_t)mDefaultVelocity; }

void EditorState::setDefaultVelocity(int8_t val)
{
    jassert(val >= 0 && val <= 127);
    mDefaultVelocity = val;
}

int EditorState::getLowestOctave() { return mLowestOctave; }
...
int EditorState::getColorTheme() { return mColorTheme; }
    ...
    */
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        else if (dynamic_cast<ToggleButton *>(innercpt) != nullptr)
        {
            ...
            tb->setColour(ToggleButton::ColourIds::tickColourId,
                          btn.contrasting(.5) /* Colours::black*/);
        }
