...
static const char *gNoteNames[SEQ_NUM_NOTE_NAMES] = {"C",  "C#", "D",  "D#", "E",  "F",
                                                     "F#", "G",  "G#", "A",  "A#", "B"};
struct Scale
{
    const char *name;
...
static Scale gScaleNames[NUM_SCALES] = {{"Major", "102034050607"},
...
static ClockDivVal gClockDivs[SEQ_NUM_CLOCK_DIVS] = {{"1/16", 1}, {"1/8", 2}, {"1/4", 4},
                                                     {"1/2", 8},  {"1", 16},  {"2", 32},
                                                     {"4", 64}
...
static ChordInfo gChords[NUM_CHORDS] = {
    {{4, 7}, "major"},      {{3, 7}, "minor"},     {{3, 6}, "dim"},      {{3, 6, 9}, "dim7"},
    {{4, 8}, "aug"},        {{4, 7, 10}, "dom7"},  {{3, 7, 10}, "min7"}, {{4, 7, 11}, "maj7"},
    {{4, 7, 9}, "maj6"},    {{3, 7, 9}, "min6"},   {{4, 8, 10}, "7#5"},  {{4, 6, 10}, "7b5"},
    {{3, 7, 11}, "Maj7b3"}, {{5, 7, 10}, "7sus4"},

};

int SeqScale::getNumNoteNames() { return SEQ_NUM_NOTE_NAMES; }

const char *SeqScale::getNoteName(int idx)
{
    jassert(idx >= 0 && idx < SEQ_NUM_NOTE_NAMES);
    return gNoteNames[idx];
}

int SeqScale::getNumScales() { return NUM_SCALES; }

const char *SeqScale::getScaleName(int idx)
{
...
const char *SeqScale::getClockDividerTextAndId(int idx, int *id)
{
...
int SeqScale::getNumChords() { return NUM_CHORDS; }
...
SeqScale::SeqScale() : mScaleInt(0), mScaleOffset(-1), mCurNote(-1) {}
...
void SeqScale::startIterateNotesInScale(const char *scale, const char *key, int start_octave)
{
    ...
    for (i = 0; i < NUM_SCALES; i++)
    {
        ...
        {
            if (strcmp(gScaleNames[i].intervals, "000000000000") !=
                0) // unlikely infinite loop later - use default
                mScaleInt = gScaleNames[i].intervals;
            break;
        }
    }
    // we might be loading these from a file at some point in which case it may not be found (which
    // is valid)
